...
## UI（`ui/`）
- `ActionMenu.gd` — 固定在屏幕底部作为当前单位指令栏，发出移动、技能、指令卡、提取、召唤、回收、等待等信号。MVP 中按钮由脚本补充创建；菜单会压缩按钮高度、自动换行、限制高度并在需要时滚动，避免长菜单超出屏幕。菜单会根据当前单位上下文收缩：非训练师只显示基础动作，训练师通过「同步率」顶层菜单展开「指令 / 提取 / 召唤」三个子菜单；同步率操作已使用后顶层按钮显示「同步率已用」并禁用。菜单按钮会根据当前单位显示真实技能名、卡牌消耗、冷却、目标类型、持续时机、当前提取形态或后备离场状态，鼠标悬停时通过 `option_hovered(description)` 让 `Battle.gd` 更新底部提示线。
...
- `CTBBar.gd` — 提供两种可切换视图：速度跑条视图和行动轴视图。行动轴视图为默认低矮顶部视图，会预测未来数次行动顺序，速度快的单位允许重复出现；速度跑条视图中每个单位对应一组标签+进度条，每帧从 `unit.current_ap` 更新显示，CTB 暂停时会显示 `WAIT`，当前可行动单位显示 `READY` 并高亮进度条，下一位行动单位显示 `NEXT`。CTB 区域有轻量背景面板，保证信息能从战场格子中分离出来。
