...
1. **战前准备** — `_build_prep_panel()` 在战斗开始前让玩家选择样板战、2 只开局宝可梦和训练师默认提取形态；第一战默认火狐兽 + 藤藤兽上场，第二样板战默认藤藤兽 + 电花鼠上场，训练师都默认预提取水跃兽。
2. **生成单位** — MVP 版本在 `_spawn_units()` 中动态创建 `UnitData` / `SkillData` / `SkillEffectData`，按战前准备生成训练师、2 只开局宝可梦、后备名单和敌方单位，并将单位注册到 `GridManager`、`CTBSystem` 和 `CTBBar`。第一战与第二样板战通过 `_spawn_level1_enemies()` / `_spawn_level2_enemies()` 切换敌人组合、出生位置和 Boss 属性。旧 `.tres` 数据仍保留，但当前样板战斗不依赖它们。
3. **开战提示** — `_build_battle_briefing_panel()` 在单位生成后、CTB 开始前显示一次样板战提示面板，说明胜利目标、属性克制、同步率、指令卡和敌方范围按钮；玩家点“开始行动”后才调用 `CTBSystem.start()`。
...
5. **玩家回合** — `Battle.gd` 显示 `ActionMenu`。玩家选择移动 / 技能 / 指令卡 / 召唤 / 回收 / 待机或结束，驱动 `ActionState` 状态机。格子点击通过 `GridManager` 的 `cell_clicked` 信号路由。空闲时点击敌人会预览敌方移动范围和最远攻击威胁；选择移动时会同时预览移动后最远技能覆盖范围。移动后若尚未使用技能、同步率操作或回收，可按 `Esc` 撤回到回合开始位置。选择技能后第一次点目标只显示伤害预览，确认后才结算攻击。非训练师使用技能后自动结束；训练师使用技能后仍可继续刷卡、召唤、回收或手动结束行动，但不能再移动。己方宝可梦可在自己行动时直接回收自身并结束行动。
6. **敌方回合** — `UnitAI.run()`（`RefCounted` 类上的静态方法）通过 `await` 延迟执行，便于玩家观察，并返回本次 AI 移动、攻击、蓄力或待机日志；`Battle.gd` 统一写入战斗日志后调用 `_end_turn()`。
7. **结束回合** — `_end_turn()` 从当前单位 AP 中扣除本回合行动条内部结算值，清除高亮和状态，调用 `CTBSystem.resume()` 重启计时。没有使用技能时默认扣 50 AP，并向玩家提示“下次行动提前50%”；大部分技能扣 100 AP 且不展示成本；少量快招/重招通过 `SkillData.ap_cost` 转译为“自身下次行动提前/推后 X%”。移动不消耗 AP。
...
## MVP 战斗系统
当前分支实现了 `.claude/mvp_design.md` 的第一版样板战，并额外提供 `.claude/level2_design.md` 的第二样板战：
- 训练师通过战前准备选择样板战、2 只宝可梦开场上场和默认提取形态；未上场的火狐兽、藤藤兽、水跃兽、电花鼠、冰羽兽会进入后备名单。默认提取形态开战时直接生效。战前准备、HUD、菜单悬停说明和日志会按“宝可梦 / 属性 / 定位 / 提取技能”展示提取形态。
- 敌方包含火属性近战、草属性近战、水属性远程、飞属性高速远程、地属性慢速近战和草属性厚血大怪。
- 第二样板战通过战前准备下拉框选择，敌方包含地属性厚血大怪、中线地属性守卫、上下翼长射程水属性远程、飞属性猎手和草属性移动压制，用于验证移动压制、属性选择和召唤取舍是否能换图成立。
- 训练师回合可消耗同步率使用固定指令卡：高速组件、小型护盾、火力插件、弱点标记、战术换位、属性校准；也可召唤后备宝可梦。每个训练师回合最多结算 1 次同步率操作，移动、技能和回收不占用这次机会。高速组件提供宝可梦下一次移动 +2，而不是补 AP。
- 后备能力提取、召唤与回收由 `Battle.gd` 的 `_pokemon_roster`、`_reserve_units`、`_reserve_hp_by_name`、`SUMMON_DEFS`、`EXTRACT_DEFS`、`_summon_count` 和 `_trainer_extract_id` 管理。战前默认提取免费生效；战斗中再次提取消耗 100 同步率。额外召唤按本场已成功召唤次数递增费用：100、160、220；回收返还 50 同步率，但不会降低下一次召唤费用。召唤和战斗中提取都需要对应宝可梦仍在后备中；召唤会让它离开后备，当前提取形态会占用对应后备，使它暂时不可召唤。切换到其他提取形态后，旧来源恢复可召唤。回收会把宝可梦放回后备并保存当前 HP，再次召唤不会满血重置。提取后训练师会切换属性，并把技能列表替换为对应宝可梦技能，直到下一次提取。`EXTRACT_DEFS` 同时提供 `element_label`、`role`、`skill_name` 作为 UI 展示源，避免提取读起来像单纯换皮指令卡。
...
- 训练师倒下不会直接失败，而是进入指挥离线状态：不能再刷卡、召唤或执行训练师回收，只能继续操作已在场的宝可梦。己方宝可梦自己的自回收仍可作为普通行动使用；己方全部单位倒下才失败。
...
- `setup_mvp_terrain(level_id)` / `set_terrain()` / `get_terrain()` — 按样板战初始化与读写 MVP 地形
...
## UI（`ui/`）
- `ActionMenu.gd` — 固定在屏幕底部作为当前单位指令栏，发出移动、技能、指令卡、提取、召唤、回收、等待等信号。指令栏左侧显示当前行动单位，按钮下方以小字显示当前提示或鼠标悬停说明。MVP 中按钮由脚本补充创建；菜单会压缩按钮高度、自动换行、限制高度并在需要时滚动，避免长菜单超出屏幕。菜单会根据当前单位上下文收缩：宝可梦显示基础动作和自回收；训练师额外通过「同步率」顶层菜单展开「指令 / 提取 / 召唤」三个子菜单，并在底部指令栏保留独立回收按钮。同步率操作已使用后顶层按钮显示「同步率已用」并禁用，但回收仍可用。菜单按钮会根据当前单位显示真实技能名、卡牌消耗、冷却、目标类型、持续时机、当前提取形态或后备离场状态。
- `Battle.gd` 动态创建轻量伤害预览面板。预览面板显示技能名、命中目标、HP 变化、伤害和总伤害；范围技能会列出所有受影响敌人，并在地图上显示预览数字。伤害预览占用右侧抽屉，打开时会临时隐藏日志。
- `Battle.gd` 动态创建行动日志抽屉，按时间顺序记录已结算行动。日志默认折叠，点击顶部“日志”按钮后在右侧展开；日志使用 `RichTextLabel` 渲染，我方单位名为蓝色、敌方单位名为红色、中立/野生单位名为柔和黄色。
- `Battle.gd` 动态创建棋盘单位悬停小卡。鼠标停在棋盘单位上时，小卡显示阵营、属性、HP/AP、速度、移动、技能概要和临时状态；鼠标在 UI 面板上方时不显示，避免与指令栏、日志和预览互相遮挡。
- `CTBBar.gd` — 提供两种可切换横轴视图。`AP轴` 是默认低矮顶部视图，所有存活单位以单字头像在同一条轴上按当前 AP 从左向右推进，最右侧代表 AP 满；`顺序` 视图按预测行动顺序把头像固定排在同一条轴上，下一位行动者在最右侧，速度快的单位允许重复出现。CTB 头像可悬停查看单位名、AP、速度、状态和预计行动位次；CTB 区域有轻量背景面板，保证信息能从战场格子中分离出来。
