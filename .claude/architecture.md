...
10. **战斗结算** — `_show_result_panel()` 在胜负判定后显示结算面板，汇总胜负、封印名单、击败敌人数、同步率操作数、训练师是否倒下和剩余同步率。击败数来自敌方死亡事件，同步率操作数来自正式战斗日志，取消和预览不计入。
...
- 战斗结束后显示结算面板，帮助玩家复盘是否封印、击败了多少敌人、用了多少同步率操作，以及训练师是否倒下。
