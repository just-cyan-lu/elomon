...
2. **生成单位** — MVP 版本在 `_spawn_units()` 中动态创建 `UnitData` / `SkillData`，按战前准备生成训练师、2 只开局宝可梦、后备名单和敌方单位，并将单位注册到 `GridManager`、`CTBSystem` 和 `CTBBar`。第一战与第二样板战通过 `_spawn_level1_enemies()` / `_spawn_level2_enemies()` 切换敌人组合、出生位置和 Boss 属性。旧 `.tres` 数据仍保留，但当前样板战斗不依赖它们。
...
## UI（`ui/`）
- `ActionMenu.gd` — 固定在屏幕底部作为当前单位指令栏，发出移动、技能、指令卡、提取、召唤、回收、等待等信号。指令栏左侧显示当前行动单位，按钮下方以小字显示当前提示或鼠标悬停说明。MVP 中按钮由脚本补充创建；菜单会压缩按钮高度、自动换行、限制高度并在需要时滚动，避免长菜单超出屏幕。菜单会根据当前单位上下文收缩：非训练师只显示基础动作，训练师通过「同步率」顶层菜单展开「指令 / 提取 / 召唤」三个子菜单；同步率操作已使用后顶层按钮显示「同步率已用」并禁用。菜单按钮会根据当前单位显示真实技能名、卡牌消耗、冷却、目标类型、持续时机、当前提取形态或后备离场状态。
