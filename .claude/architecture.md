...
- `CTBBar.gd` — 提供两种可切换横轴视图。`AP轴` 是默认低矮顶部视图，所有存活单位以单字头像在同一条轴上按当前 AP 从左向右推进，最右侧代表 AP 满；`顺序` 视图按预测行动顺序把头像固定排在同一条轴上，下一位行动者在最右侧，速度快的单位允许重复出现。CTB 区域有轻量背景面板，保证信息能从战场格子中分离出来。
