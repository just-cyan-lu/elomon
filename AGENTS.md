...
| 战斗结算 | `battle/Battle.gd` | 胜负后显示封印、击败数、同步率操作、训练师状态和剩余同步率 |
| 全局枚举/常量 | `core/Enums.gd` | `UnitType`、`BattleState`、`ActionState`、网格常量 |

同步率当前没有硬上限。自然回复只补到 100，训练师攻击、宝可梦攻击、捕捉等主动收益可以继续超过 100；同步率暂不跨战斗继承。

训练师初始同步率为 100。每个训练师回合最多结算 1 次同步率操作：指令卡、提取、召唤、回收和封印互斥；移动和技能不占用这次机会。训练师菜单中这些操作统一收在「同步率」顶层菜单下，再展开「指令 / 提取 / 召唤」。

战斗开始前会显示轻量战前准备面板：选择 2 只开局宝可梦和训练师默认提取形态。默认配置为火狐兽 + 藤藤兽上场，训练师默认提取水跃兽；战斗中再次提取消耗 100 同步率。

战前准备确认后会先显示一次“第一场战斗”提示面板，说明胜利目标、属性 2 倍克制、红血 100% 封印、同步率操作限制和敌方范围按钮；玩家点“开始行动”后 CTB 才开始推进。
...
当前 MVP 暂不展示稳定度，也不把稳定度作为捕捉条件。封印捕捉只看野生单位 HP：HP 越低成功率越高，血条进入红色区间时 100% 成功。

战斗结束后会显示结算面板。结算只统计真实落地的事件：击败数来自单位死亡，封印来自 `_captured_names`，同步率操作来自正式战斗日志，不统计预览、取消或未提交的 pending 日志。
